...
\subsection{Organización y estructura del código}

El proyecto se ha estructurado en paquetes para separar responsabilidades y facilitar el mantenimiento. La distribución principal es la siguiente:
...
\begin{itemize}
    ...
    \item \texttt{run-gui.sh}, \texttt{stop-gui.sh}: scripts de arranque y parada.
\end{itemize}

Más abajo se añade un diagrama más detallado de la estructura de directorios y de la relación entre clases y paquetes
...
\begin{figure}[H]
    \centering
        \includegraphics[width=0.85\linewidth]{diagrama_clases.png}
...
\begin{itemize}
    ...
    \item \textbf{Ajustes de cuenta:} se incluyeron ajustes propios (cerrar sesi\'on, cambiar contrase\~na) para centralizar la gesti\'on del usuario y evitar men\'us dispersos.
...
Para ejecutar la aplicación, basta con situarse en la carpeta del proyecto y lanzar el script de arranque:
...
\begin{enumerate}
    \item Inicia sesión con una cuenta válida. Para entrar como alumno: elena.ruiz@uco.es; mientras que para entrar como moderador: moderador@uco.es. En ambos casos la contraseña es 1234.
